...
PATH_DIC = {
    'cephalometric': '/home/kui/Qikui/landmark_detection/Dataset/ISBI2015_ceph/AnnotationsByMD/raw',
    'hand': '/home/kui/Qikui/landmark_detection/Dataset/hand/jpg',
    'chest': '/home/kui/Qikui/landmark_detection/Dataset/chest/pngs',
}
...
def evaluate(input_path, output_path, save_img=True, assigned=False, IS_DRAW_TEXT=False):
    ...
    for i, gt_p in enumerate(pbar):
        ...
        #print(heatmaps.shape, gt_map.shape)
        ...
        #print(cur_distance_list, cur_pixel_dis)
        
        xx = sample_analysis(cur_pixel_dis, name)
        print(xx['SDR'])
        dic['MRE'].append(xx['MRE'])
        dic['2mm'].append(xx['SDR'][2])
        dic['2.5mm'].append(xx['SDR'][2.5]) 
        dic['3mm'].append(xx['SDR'][3])  
        dic['4mm'].append(xx['SDR'][4]) 
    ...
    if assigned:
        print('assigned...')
    toYaml(output+'/sample_summary.yaml', dic)
    print('!!!!!!!!!!!!!!', np.mean(dic['MRE']), np.mean(dic['2mm']))
...
def sample_analysis(li1, dataset):
    print('\n'+'-'*20+dataset+'-'*20)
    summary = {}
    mean1, std1, = np.mean(li1), np.std(li1)
    sdr1 = get_sdr(li1)
...
def get_args():
    ...
    # optinal
    parser.add_argument("-s", "--save_img", default=True, action='store_true')
    parser.add_argument("-d", "--draw_text", default=True,action='store_true')
    parser.add_argument("-o", "--output", type=str)
    parser.add_argument("-C", "--config", type=str)
    parser.add_argument("-a", "--assigned", action='store_true')
    # required
    parser.add_argument("-i", "--input", type=str, default='/media/kui/porter/CASE/LandMark/Unet_loss/chest_CE/unet2d/results/test_epoch015/')
...
if __name__ == "__main__":
    ...
    pixel_dic = {}
    if not args.output:
        output = os.path.join('./RE/', args.input.replace('/', '_'))
